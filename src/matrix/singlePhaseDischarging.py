...
class GetArrays:
    ...
    def coeffMatrix(self, props):
        ...
        coefficients[0][0] = 4*k[0]*k[1]/(k[0]+k[1])*props.dt + 2*props.G*cGas[0]*props.dz*props.dt + thermalMass[0]*(props.dz**2)
        coefficients[0][1] = -4*k[0]*k[1]/(k[0]+k[1])*props.dt - 2*props.G*cGas[1]*props.dz*props.dt
        coefficients[-1][-2] = -4*k[-2]*k[-1]/(k[-2]+k[-1])*props.dt + props.G*cGas[-2]*props.dz*props.dt
        coefficients[-1][-1] = 4*k[-2]*k[-1]/(k[-2]+k[-1])*props.dt + 2*props.h*props.dz*props.dt + thermalMass[-1]*(props.dz**2)
    ...
    def constArray(self, props, previousTemperatures):
        ...
        constants[0] = thermalMass[0]*(props.dz**2)*previousTemperatures[0]
        constants[-1] = thermalMass[-1]*(props.dz**2)*previousTemperatures[-1] + (2*props.h*props.dz*props.dt + props.G*cGas[-1]*props.dz*props.dt)*props.Tambient
