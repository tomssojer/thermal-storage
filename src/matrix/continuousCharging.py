...
class GetArrays:
    ...
    def coeffMatrix(self, props):
        ...
        for i in range(1, halfArray-1):
            ...
            coefficients[i][i+1] = -2*(kSolid[i]*kSolid[i+1])/(kSolid[i]+kSolid[i+1])*props.dt
        ...
        # Fluid - 1. robni pogoj
        # Iz enačb k_fluid sem kar enačil s k0, ker vrednosti jemljem iz ene datoteke in jih shranim v arrayu
        # Tako da predpostavim k_fluid = k0, pravi vrednosti so namreč skoraj enaki
        ...
        for i in range(halfArray+1, wholeArray-1):
            ...
            coefficients[i][i+1] = props.voidFrac*rhoGas[i]*cGas[i]*props.flowVelocity*props.dt*props.dz - 4*kFluid[i]*kFluid[i+1]/(kFluid[i]+kFluid[i+1])*props.dt

        # Fluid - 2. robni pogoj
        coefficients[wholeArray-1][halfArray-1] = -props.h*A*props.dt*props.dz**2
        coefficients[wholeArray-1][wholeArray-2] = -props.voidFrac*rhoGas[wholeArray-2]*cGas[wholeArray-2]*props.flowVelocity*props.dt*props.dz - 2*kFluid[wholeArray-1]*kFluid[wholeArray-2]/(kFluid[wholeArray-1]+kFluid[wholeArray-2])*props.dt
        coefficients[wholeArray-1][wholeArray-1] = props.voidFrac*rhoGas[wholeArray-1]*cGas[wholeArray-1]*props.dz**2 + props.voidFrac*rhoGas[wholeArray-1]*cGas[wholeArray-1]*props.flowVelocity*props.dt*props.dz + 2*kFluid[wholeArray-1]*kFluid[wholeArray-2]/(kFluid[wholeArray-1]+kFluid[wholeArray-2])*props.dt + props.h*A*props.dt*props.dz**2
